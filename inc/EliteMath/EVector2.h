...
#define UnitVector2 Vector2(1.f,1.f)
	...
	struct Vector2
	{
		...
		{
			const float magSqred = MagnitudeSquared();
			const float vMagSqred = v.MagnitudeSquared();
			if (AreEqual(magSqred, vMagSqred))
			{
				if (AreEqual(x, v.x))
					return y < v.y;
				return x < v.x;
		...
		// 90-degree perpendicular rotation (counter-clockwise)
		static inline Vector2 Perpendicular(const Vector2& v) {
			return {-v.y, v.x};
		}
